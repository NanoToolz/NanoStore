...
# Go to project root (one level up from deploy/)
cd "$(dirname "$0")/.."
...
if podman build -t nanostore-bot -f deploy/Dockerfile . ; then
...
echo -e "${CYAN}Restart:${NC}      podman restart nanostore-bot"
echo -e "${CYAN}Status:${NC}       podman ps"
echo ""
